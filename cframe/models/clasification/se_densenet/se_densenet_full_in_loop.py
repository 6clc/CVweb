...
def se_densenet201(configer):
    pretrained = configer['pretrained']
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SEDenseNet(num_init_features=64, growth_rate=32, block_config=(6, 12, 48, 32), num_classes=num_classes)
    if pretrained:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
        # They are also in the checkpoints in model_urls. This pattern is used
        ...
        #         state_dict[new_key] = state_dict[key]
        #         del state_dict[key]
        state_dict = torch.load('/home/he/.SummaryWriter.bak/SummaryWriter/SummaryWriter/DSN/se_densenet201_pretrained(esos04)_instance.json/best_model.pth')
        print('load pretrainde parameters from {}'.format('se_densenet201_pretrained esos04'))
        model.load_state_dict(state_dict, strict=False)
    return model


def se_densenet161(configer):
    ...
    pretrained = configer['pretrained']
    num_classes = configer['num_classes']
    model = SEDenseNet(num_init_features=96, growth_rate=48, block_config=(6, 12, 36, 24), num_classes=num_classes)
    if pretrained:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
        # They are also in the checkpoints in model_urls. This pattern is used
