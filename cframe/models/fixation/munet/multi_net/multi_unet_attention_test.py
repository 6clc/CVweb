...
class MultiUnetAttentionTest(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(s.shape, sum(self.channels))
        s = self.attention(s)
        ...
        decoder_out = self.decoders[idx](encoder_outs[:i+1])
        return decoder_out, idx


if __name__ == '__main__':
    config = dict(nblocks=5, in_planes=3, num_classes=1,
                  channels=[64, 128, 256, 512, 512])
    net = MultiUnetAttentionTest(config)
